                ...
                if existing_proxy is None:
                    ...
                    db.execute(
                        ...
                        UPDATE provider_assignments SET subscription_id=?, host=?, port=?, username_encrypted=?,
                            ...
                            dashboard_assignment_id=CASE WHEN ? THEN NULL ELSE dashboard_assignment_id END,
                            dashboard_eligible=CASE WHEN ? THEN NULL ELSE dashboard_eligible END,
                            dashboard_connections=CASE WHEN ? THEN NULL ELSE dashboard_connections END,
                            dashboard_observed_at=CASE WHEN ? THEN NULL ELSE dashboard_observed_at END,
                            dashboard_source=CASE WHEN ? THEN '' ELSE dashboard_source END,
                            dashboard_error_code=CASE WHEN ? THEN '' ELSE dashboard_error_code END,
